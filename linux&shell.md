...
　　即便是现代文件系统，碰上突然断电或者某个不规矩的程序在访问文件时锁定了系统，也会出现错误。幸而有一些命令行工具可以帮你将文件系统恢复正常。
　　每个文件系统都有各自可以和文件系统交互的恢复命令。好在有个通用的前端程序，可以决定存储设备上的文件系统并根据要恢复的文件系统调用合适的文件系统恢复命令。
　　fsck命令能够检查和恢复大部分类型的Linux文件系统，包括ext、ext2、ext3、ext4、ReiserFS、JFS和XFS。该命令的格式是：

	fsck options filesystem

　　可以在命令行列出多个要检查的文件系统。文件系统可以通过设备名、在虚拟目录中的挂载点以及分配给文件系统的唯一UUID值来引用。
　　fsck命令使用/etc/fstab文件来自动决定正常挂载到系统上的存储设备的文件系统。如果存储设备尚未挂载，则需要用-t命令行选项来指定文件类型。以下是其他可用的命令行选项：

- -a：若检测到错误，自动修复文件系统
- -A：检查/etc/fstab文件中列出的文件系统
- -C：给支持进度条功能的文件系统显示出一个进度条（只有ext2和ext3）
- -N：不进行检查，只显示哪些检查会执行
- -r：出现错误时提示
- -R：使用-A选项时条多根文件系统
- -s：检查多个文件系统时，一次进行检查
...
　　其中有些命令行选项时重复的，这是为多个命令实现通用的前端而带来的部分问题。有些文件系统恢复命令有一些额外的可用选项。若要做更高级的错误检查，就需要查看这个文件系统修复工具的手册页面来确定是否有该文件系统专用的扩展选项。
　　fsck命令只能在未挂载的文件系统上运行。对大多数文件系统来说，只需卸载文件系统来进行检查，检查完成后重新挂载即可。但因为根文件系统含有所有核心的Linux命令和日志文件，所以无法在处于运行状态的系统上卸载。
　　分区大小只能在物理硬盘的可用空间范围内调整。当磁盘空间不足时，可以通过将另外一个硬盘上的分区加入已有文件系统，动态地添加存储空间。Linux逻辑卷管理器（logical volume manager，LVM）软件包正好可以用来做这个。它可以在无需重建整个文件系统的情况下，管理磁盘空间。
　　逻辑卷管理的核心在于如何处理安装在系统上的硬盘分区。在逻辑卷管理的世界里，硬盘称为物理卷（physical volume，PV）。每个物理卷都会映射到磁盘上特定的物理分区。
　　多个物理卷集中在一起可以形成一个卷组（volume group，VG）。逻辑卷管理系统将卷组视为一个物理磁盘，但事实上卷组可能是由分布在多个物理磁盘上的多个物理分区组成的。卷组提供了一个创建逻辑分区的平台，而这些逻辑分区则包含了文件系统。
　　整个结构中的最后一层是逻辑卷（logical volume，LV）。逻辑卷为Linux提供了创建文件系统的分区环境，作用类似于物理磁盘分区。Linux系统将逻辑卷视为物理分区。
　　可以使用任意一种标准Linux文件系统来格式化逻辑卷，然后再将它加入Linux虚拟目录的某个挂载点。

![](https://i.imgur.com/E9zcSfC.png)

　　上图显示了典型Linux逻辑卷管理环境的基本布局，其中卷组横跨了三个不同的物理硬盘，覆盖了五个独立的物理分区。在卷组内部有两个独立的逻辑卷。Linux系统将每个逻辑卷视为一个物理分区。每个逻辑卷可以被格式化成ext4文件系统，然后挂载到虚拟目录中的特定位置。
　　在第三个物理硬盘有一个未使用的分区。通过逻辑卷管理，随后可以将这个未使用的分区分配到已有卷组，要么用它创建一个新的逻辑卷，要么在需要更多空间时用它来扩展已有的逻辑卷。类似地，如果系统新添了一块硬盘，逻辑卷管理系统允许将它添加到已有卷组，为某个已有的卷组创建更多空间，或是创建一个可用来挂载的新逻辑卷。